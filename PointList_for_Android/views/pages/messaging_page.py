...
class MessagingPage(BasePage):
    ...
    def __init__(self, page: ft.Page):
        ...
        self._body_container_ref = ft.Ref[ft.Container]()
    ...
    def _trigger_apk_notification(self, sender_name: str, msg_content: str):
        """Dispara una notificación flotante en la app y en la cortina de estado de Android (incluso en segundo plano)."""
        clean_msg = msg_content[:60] if msg_content else "Nuevo archivo recibido"
        
        # 1. Notificación nativa en la cortina de Android (Barra de estado)
        ...
        # 2. Toast / SnackBar flotante dentro de la app
        try:
            if self.page:
                snack = ft.SnackBar(
                    content=ft.Row([
                        ...
                        ft.Column([
                            ...
                            ft.Text(clean_msg, color="white", size=11),
    ...
    def _file_to_base64_url(self, path: str) -> str:
        """Convierte una imagen local a Data URL base64 para envío universal entre dispositivos."""
        try:
            if not path: return ""
            if path.startswith("data:image/") or path.startswith("http"):
                return path
            if os.path.isfile(path):
                ext = path.lower().rsplit(".", 1)[-1] if "." in path else "png"
                mime = "jpeg" if ext in ("jpg", "jpeg") else ("png" if ext == "png" else "gif")
    ...
    def _send_message_with_attachment(self, file_path, file_name):
        ...
        payload_path = self._file_to_base64_url(file_path) if media_kind == "image" else file_path
        ...
        def save_task():
            try:
                self._db.guardar_mensaje(self._uid, contact_id, new_msg["content"], payload_path)
            except:
                pass
    ...
    def _build_message_bubble(self, msg: dict, is_sender: bool, colors: dict) -> ft.Container | None:
        ...
        if file_path and msg_type in ("image", "video", "file"):
            parts: list = []
            if msg_type == "image":
                img_src = file_path or content
                if img_src and (img_src.startswith("data:image/") or img_src.startswith("http") or os.path.isfile(img_src)):
                    parts.append(
                        ft.Image(
                            src=img_src,
                            width=260,
                ...
                elif img_src and len(img_src) > 100:
                    try:
                        parts.append(
                            ft.Image(
                                src_base64=img_src,
    ...
    def _select_contact(self, contact):
        ...
        if self.is_mobile():
            if self._body_container_ref.current:
                self._body_container_ref.current.content = right_p
                try:
                    self._body_container_ref.current.update()
                except:
                    pass
            else:
            ...
            if self._main_row_ref.current and len(self._main_row_ref.current.controls) > 1:
                self._main_row_ref.current.controls[1] = right_p
            try:
                self.page.update()
            except:
                pass
