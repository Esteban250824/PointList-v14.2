...
class ChatBotPage(BasePage):
    ...
    def _sync_sessions_background(self):
        """Sincroniza sesiones en background."""
        time.sleep(0.5)
        while not self._stop_sync:
            try:
                ...
                if new_sessions != self._sessions:
                ...
                if self._current_session_id:
                    ...
                    if len(new_history) != len(cached_history) or (new_history and cached_history and new_history[-1] != cached_history[-1]):
    ...
    def _select_session(self, session_id: str, update_ui=True):
        ...
        self.page.client_storage.set(f"last_chatbot_session_{self._uid}", session_id)
    ...
    def _build_session_item(self, session: dict) -> ft.Container:
        ...
        def _show_floating_menu(e):
            """Despliega un menú contextual flotante en la posición del cursor."""
            x = e.global_x
            y = e.global_y
            ...
            # Detector para cerrar el menú si se hace clic afuera
    ...
    def _open_mobile_history(self, e=None):
        ...
        # Refrescar las sesiones desde la base de datos
        try:
    ...
    def _show_session_options_modal(self, session: dict):
        """Muestra menú modal de opciones al mantener presionado una sesión por 3 segundos."""
        sid = session["session_id"]
    ...
    def _build_session_item(self, session: dict) -> ft.Container:
        ...
        return ft.GestureDetector(
            on_secondary_tap_down=_show_floating_menu if False else None,
    ...
    def _clear_attached_file(self, file_idx=None):
        if file_idx is not None and 0 <= file_idx < len(self._attached_files):
            self._attached_files.pop(file_idx)
        else:
            self._attached_files = []
        self._refresh_file_badges()

    def _refresh_file_badges(self):
        if not self._attached_files:
            ...
            for idx, item in enumerate(self._attached_files):
                badge_controls.append(
                    ft.Container(
                        content=ft.Row([
                            ft.Icon(ft.Icons.INSERT_DRIVE_FILE, size=14, color="#00E676"),
                            ft.Text(item["name"], size=11, weight=ft.FontWeight.BOLD, color="#1E293B"),
                            ft.IconButton(icon=ft.Icons.CLOSE, icon_size=12, icon_color="#64748B", on_click=lambda e, i=idx: self._clear_attached_file(i))
            ...
            self._file_badge.content = ft.Row(controls=badge_controls, scroll=get_scroll_mode("AUTO"), spacing=6)
        try: self.page.update()
        except: pass
            ...
            if ext in [".png", ".jpg", ".jpeg", ".webp", ".bmp"]:
                ...
                try:
                    ...
                    with open(f_path, "rb") as img:
                        ...
                        f_content = f"data:{mime};base64,{b64}"
                except Exception as ex:
                    f_content = f"Error al cargar imagen: {ex}"
            elif ext in [".docx", ".doc"]:
                f_type = "documento_word"
                try:
                    import docx
                    ...
                    try:
                        ...
                        f_content = " ".join(texts)[:12000]
                    except Exception as ex:
                        f_content = f"Documento de Word: {f_name}"
            elif ext == ".pdf":
                f_type = "documento_pdf"
                try:
                    import pypdf
                    reader = pypdf.PdfReader(f_path)
                    pages_txt = [p.extract_text() for p in reader.pages[:10] if p.extract_text()]
                    f_content = "\n".join(pages_txt)[:12000]
                except:
                    try:
                        with open(f_path, "rb") as f:
                            raw = f.read(40000)
                            import re
                            text_chunks = re.findall(rb"[\x20-\x7E\s]{4,}", raw)
                            f_content = "\n".join([chunk.decode('latin1', errors='ignore') for chunk in text_chunks if len(chunk) > 8])[:10000]
                    except Exception as ex:
                        f_content = f"Error extrayendo PDF: {ex}"
            else:
                f_type = "documento_texto"
                try:
                    with open(f_path, "r", encoding="utf-8", errors="replace") as f:
                        f_content = f.read(12000)
                except Exception as ex:
                    f_content = f"Contenido del archivo {f_name}"
    ...
    def _send_message(self, e):
        ...
        if not query and not self._attached_files: return
        ...
        display_query = query
        current_attached_list = list(self._attached_files)
        if current_attached_list:
            file_names = ", ".join([f["name"] for f in current_attached_list])
            file_label = f"📎 [Adjuntos ({len(current_attached_list)}): {file_names}]"
            display_query = f"{file_label}\n{query}" if query else file_label
            self._clear_attached_file()

        self._chat_list.controls.append(self._build_message_bubble(display_query, is_user=True))
        ...
        def _process():
            try:
                response = chatbot.send_message(query, self._uid, sid, history=history, attached_files=current_attached_list)
                ...
                if is_first_message:
                    title_src = query or (current_attached['name'] if current_attached else "Conversación")
    ...
    def build(self) -> ft.Control:
            ...
            content=ft.Column([
                ...
                ft.Row([
                    ft.IconButton(
                        ...
                        icon_color="#64748B",
                        tooltip="Adjuntar múltiples archivos (PDF, Word, Imagen, Video, Código)",
                        on_click=lambda e: self._file_picker.pick_files(allow_multiple=True),
                    ),
        ...
        # Decidir qué mostrar en el centro: historial o tarjetas de ayuda inicial
        ...
        # 3. PANEL DE AYUDA DERECHO (Verde Pastel - solo visible si no hay historial)
        # Se oculta cuando el usuario ya tiene conversaciones previas
